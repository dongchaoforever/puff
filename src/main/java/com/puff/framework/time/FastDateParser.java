...
public class FastDateParser implements DateParser, Serializable {
	 ...
	 */
	private static StringBuffer escapeRegex(final StringBuffer regex, final String value, final boolean unquote) {
		regex.append("\\Q");
		for (int i = 0; i < value.length(); ++i) {
			...
			case '\\':
				...
				/*
				 * If we have found \E, we replace it with \E\\E\Q, i.e. we stop
				 * the quoting, quote the \ in \E, then restart the quoting.
				 *
				 * Otherwise we just output the two characters. In each case the
				 * initial \ needs to be output and the final char is done at
				 * the end
				 */
	...
	private static abstract class Strategy {
		...
		/**
		 * Generate a <code>Pattern</code> regular expression to the <code>StringBuffer</code>
		 * which will accept this field
		 * @param parser The parser calling this strategy
		 * @param regex The <code>StringBuffer</code> to append to
		 * @return true, if this field will set the calendar;
		 * false, if this field is a constant value
		 */
		abstract boolean addRegex(FastDateParser parser, StringBuffer regex);
	}
	...
	private static class CopyQuotedStrategy extends Strategy {
		 ...
		 */
		@Override
		boolean addRegex(final FastDateParser parser, final StringBuffer regex) {
	...
	private static class NumberStrategy extends Strategy {
		...
		@Override
		boolean addRegex(final FastDateParser parser, final StringBuffer regex) {
			// See LANG-954: We use {Nd} rather than {IsNd} because Android does
			// not support the Is prefix
